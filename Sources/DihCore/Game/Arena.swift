...
public enum DihArena {
  ...
  public static func clampedCenter(in arena: CGRect, buttonSize: CGSize, desired: CGPoint)
    -> CGPoint
  {
  ...
  public static func randomCenter(
    in arena: CGRect, buttonSize: CGSize, generator: inout some RandomNumberGenerator
  ) -> CGPoint {
    let halfWidth = min(buttonSize.width / 2, arena.width / 2)
    let halfHeight = min(buttonSize.height / 2, arena.height / 2)
    let x = CGFloat.random(
      in: (arena.minX + halfWidth)...(arena.maxX - halfWidth), using: &generator)
    let y = CGFloat.random(
      in: (arena.minY + halfHeight)...(arena.maxY - halfHeight), using: &generator)
