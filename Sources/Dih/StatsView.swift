        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                ...
                LazyVGrid(columns: [GridItem(.adaptive(minimum: 150), spacing: 12)], spacing: 12) {
                    ...
                    StatCard(title: "Total earned", value: "\(game.save.totalPointsEarned)", icon: "chart.line.uptrend.xyaxis")
                    ...
                    StatCard(title: "Accuracy", value: game.accuracy.formatted(.percent.precision(.fractionLength(1))), icon: "scope")
                    ...
                    LazyVGrid(columns: [GridItem(.adaptive(minimum: 180), spacing: 8)], spacing: 8) {
                        ForEach(DihAchievementID.allCases, id: \.self) { achievement in
                            ...
                            Label(achievement.title, systemImage: unlocked ? achievement.icon : "lock.fill")
                                ...
                                .background(unlocked ? Color.orange.opacity(0.12) : Color.secondary.opacity(0.12), in: RoundedRectangle(cornerRadius: 7))
                        }
                    }
                }
            }
